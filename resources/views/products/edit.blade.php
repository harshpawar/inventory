<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-4" id="product-form">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-sm">Name</label>
                        <input name="name" value="{{ old('name', $product->name) }}" class="mt-1 w-full border rounded p-2">
                        <x-input-error :messages="$errors->get('name')" />
                    </div>
                    <div>
                        <label class="block text-sm">Code</label>
                        <input name="code" value="{{ old('code', $product->code) }}" class="mt-1 w-full border rounded p-2">
                        <x-input-error :messages="$errors->get('code')" />
                    </div>
                    <div>
                        <label class="block text-sm">Unit Cost</label>
                        <input type="number" step="0.000001" name="unit_cost" value="{{ old('unit_cost', $product->unit_cost) }}" class="mt-1 w-full border rounded p-2">
                        <x-input-error :messages="$errors->get('unit_cost')" />
                    </div>
                    <div>
                        <label class="block text-sm">Description</label>
                        <textarea name="description" class="mt-1 w-full border rounded p-2">{{ old('description', $product->description) }}</textarea>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="border-b p-2">
                                        <select class="border rounded p-2 w-full" name="components[{{ $i }}][inventory_item_id]">
                                            @foreach ($inventory as $item)
                                                <option value="{{ $item->id }}" @selected($item->id == $component->inventory_item_id)>{{ $item->name }} ({{ $item->sku }})</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="border-b p-2">
                                        <input type="number" step="0.000001" class="border rounded p-2 w-full" name="components[{{ $i }}][quantity]" value="{{ rtrim(rtrim(number_format($component->quantity, 6, '.', ''), '0'), '.') }}">
                                    </td>
                                    <td class="border-b p-2">
    ...
    <script>
        let idx = {{ count($product->components) }};
        function addRow() {
