<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">New Inventory Item</h2>
    </x-slot>
    <div class="py-6">
            ...
            <div class="bg-white shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('inventory-items.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="sku" value="{{ old('sku') }}" class="mt-1 w-full border rounded p-2">
                        <x-input-error :messages="$errors->get('sku')" />
                    </div>
                    <div>
                        <label class="block text-sm">Unit</label>
                        <input name="unit" value="{{ old('unit', 'pcs') }}" class="mt-1 w-full border rounded p-2">
                        <x-input-error :messages="$errors->get('unit')" />
                    </div>
                    <div>
                        <label class="block text-sm">Initial Stock</label>
                        <input type="number" step="0.000001" name="stock" value="{{ old('stock', 0) }}" class="mt-1 w-full border rounded p-2">
